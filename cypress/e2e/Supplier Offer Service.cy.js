describe('Supplier Offer RFQ (Product)', () => {
    ...
    it('test1-verify supplier_offer', () => {
        ...
        cy.get('.baUaEkaC1 > .Input').should('be.visible').and('exist').type('100')
        cy.get('.Dropdown').should('be.visible').and('exist').select('100% Upon Completion')
        //cy.get('.baTtaSy > .MultiLineInput').should('be.visible').and('exist').type('Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque aliquet libero mi, ac vulputate velit cursus sed. Pellentesque sed suscipit leo. Nullam feugiat varius elit id luctus. Nam ac dui vitae massa sollicitudin maximus. Nulla vulputate leo ut mi imperdiet congue. Suspendisse ipsum libero, euismod ac laoreet ut, iaculis sit amet lorem. Phasellus vitae fermentum enim, et semper enim. Aliquam id est a mauris bibendum placerat sed eu augue.')
        cy.get('.baUaEjaX1 > .multifileupload-MultiFileInput > .dropzone > .dz-default').should('be.visible').and('exist').attachFile('rizal6.jpg', { subjectType: 'drag-n-drop' })
        cy.get('.baUaEjaR1 > .multifileupload-MultiFileInput > .dropzone > .dz-default').should('be.visible').and('exist').attachFile('document.pdf', { subjectType: 'drag-n-drop' })
        .wait(3000)
        cy.get('.baUaEkaM1').should('be.visible').and('exist').click()
